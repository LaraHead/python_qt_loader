################################
### загрузчик dubli стоит в планировщике, без запуска SQL запросов,   их вручную после отработки скрипта
##############################
import os
import sys
import shutil
...
logs_file = config['DUBL_logging']['logs_file']
...
###################################################################################################################
def CopyFromPost ():
    source_folder_path = config['DUBL_path']['source_folder_path']
    if check_network_drive_access(source_folder_path):
        specific_filename_pattern = config['DUBL_path']['specific_filename_pattern']
        ...
        if matching_files:
            ...
            if latest_file:
                ...
                destination_folder = config['DUBL_path']['destination_folder']
